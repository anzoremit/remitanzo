...
#if defined(NDEBUG)
# error "Remitanzocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe19ec88d541e816bf4736a0d414133fc2d601dc9392115366ca0bdbf30ceecf9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Remitanzocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Remitanzocoin Signed Message:\n";
...
{
    // Remitanzocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 84000000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 84000000); // Remitanzocoin: 84000k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 60; // Remitanzocoin: 2 minutes
static const int64 nTargetSpacing = 1 * 30; // Remitanzocoin: half minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Remitanzocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Remitanzocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Times 05/Oct/2011 Steve Jobs, Apple’s Visionary, Dies at 56";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1491673060;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2216373;
        ...
        assert(block.hashMerkleRoot == uint256("0xc4c296db71d20e37c06df7c9126cdde514a7c45e27d9a66733662a029096e3be"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Remitanzocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RemitanzocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RemitanzocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RemitanzocoinMiner(CWallet *pwallet)
{
    printf("RemitanzocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("remitanzocoin-miner");
    ...
    try { loop {
        ...
        printf("Running RemitanzocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RemitanzocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RemitanzocoinMiner, pwallet));
}
